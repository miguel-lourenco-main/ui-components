...
const gridClassName = "grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6"
...
export default function PopularComponentsGrid() {
  ...
  return (
    <StaggerGroup className={gridClassName}>
      {items.map((component) => (
        <StaggerItem key={component.name}>
          <Link href={component.href} className={cardLinkClassName}>
      ...
      <StaggerItem className="h-full">
        <Link href="/components" className={`${cardLinkClassName} h-full`}>
          <GlowCard className="flex flex-col h-full cursor-pointer">
            <CardHeader>
  ...
  return (
    <div className={gridClassName}>
      {Array.from({ length: 6 }).map((_, index) => (
        <GlowCard key={index} interactive={false}>
          <CardHeader>
            <Skeleton className="h-6 w-3/4 mb-2" />
            <Skeleton className="h-4 w-1/2" />
          </CardHeader>
          <CardContent>
            <div className="h-28 border rounded-md flex items-center justify-center bg-muted/30 p-4">
